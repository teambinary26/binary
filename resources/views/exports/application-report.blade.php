...
<body>
    ...
    <table>
        ...
        <tbody>
            @forelse ($applications as $row)
                <tr>
                    ...
                    <td>{{ $row->applicant?->primaryAddress?->barangay }}</td>
                    <td>{{ $row->status->label() }}</td>
                    <td>{{ gov_date($row->submitted_at ?? $row->created_at) }}</td>
                    <td class="amount">{{ $row->approved_amount !== null ? peso($row->approved_amount, false) : '—' }}</td>
                </tr>
            @empty
                <tr>
                    <td class="empty" colspan="7">No applications match the current filter.</td>
                </tr>
            @endforelse
        </tbody>
        @if($applications->isNotEmpty())
            <tfoot>
                <tr>
                    <td colspan="6">Total approved amount</td>
